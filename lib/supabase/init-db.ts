...
-- Children policies
CREATE POLICY "Caretakers can view their children" ON children
  FOR SELECT USING (auth.uid() = caretaker_id);

CREATE POLICY "Caretakers can insert their children" ON children
...
-- Reports policies
CREATE POLICY "Doctors can view their reports" ON reports
...
CREATE POLICY "Users can upload media for their children" ON storage.objects
  FOR INSERT WITH CHECK (
    bucket_id = 'media' AND
    auth.uid()::text = (storage.foldername(name))[1]
  );

CREATE POLICY "Users can view media for their children" ON storage.objects
  FOR SELECT USING (
    bucket_id = 'media' AND
    auth.uid()::text = (storage.foldername(name))[1]
  );
